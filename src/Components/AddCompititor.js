...
const AddPage = () => {
  ...
  const handlecompetitorSelection = async () => {
    try {
      if (selectedcomId) {
        // Make sure selectedcomId is not null
        ...
        if (competitorDoc.exists()) {
          ...
          if (Array.isArray(competitorData.poolid)) {
            // Add the selected poolId to the array if it doesn't already exist
        ...
        console.error('Invalid competitor ID for updating poolid.');
      }
    } catch (error) {
...
<Modal show={showModal} onHide={handleCloseModal}>
        <Modal.Header closeButton>
          <Modal.Title>Select Pool ID</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Select
